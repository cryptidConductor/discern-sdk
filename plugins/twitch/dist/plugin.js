...
var WelcomeMessageSession = z.object({
  id: z.string(),
  status: z.string(),
  // this should almost always be "connected"
...
var reconnectMessageSession = z.object({
  id: z.string(),
...
var SubscriptionRequest = z.object({
  type: z.string(),
  version: z.string().default("1"),
  condition: z.record(z.string())
});
var ConfigureRequest = z.discriminatedUnion("action", [
  z.object({
...
// src/connection.ts
var DEFAULT_TWITCH_EVENTSUB_URL = new URL(
  "wss://eventsub.wss.twitch.tv/ws?keepalive_timeout_seconds=10"
);
var BUFFER_SIZE = 64;
var BUFFER_HIGH_WATER_MARK = 48;
var Connection = class {
   ...
   */
  constructor(url = DEFAULT_TWITCH_EVENTSUB_URL) {
  ...
  #activateTimers() {
    ...
    this.#heartBeatTimers.push(
      globalThis.setTimeout(() => {
        ...
        console.warn(
          `heartbeat missed (${timerEnd - timerStart}ms since timer start); we'll see if we get another before reconnecting...`
        );
      }, this.#heartBeatInterval * 1e3 + 100)
    );
    this.#heartBeatTimers.push(
      globalThis.setTimeout(() => {
        const timerEnd = Date.now();
        console.error(
          `heartbeat missed (${timerEnd - timerStart}ms), we'll have to reconnect now!`
        );
        this.#postMortem(4905);
      }, this.#heartBeatInterval * 2 * 1e3 + 50)
    );
  }
  #resetTimers() {
    this.#clearTimers();
...
// src/token.ts
var twitchApiResponse = z.object({
...
var Token = class _Token {
  ...
  static async load(secretId) {
    ...
    if (secret?.data.type === "oauthToken" && isTwitchMeta(secret.metadata)) {
      return new _Token(secretId, secret.data.value);
    } else {
      throw new Error("Invalid secret data?");
    }
  }
  constructor(secret, authToken) {
    this.#secret = secret;
    this.#authToken = authToken;
  }
  async accessToken() {
    if (this.accessTokenExpiresAt.getTime() - 5e3 < Date.now()) {
  ...
  get refreshTokenExpiresAt() {
    return this.#authToken.refreshTokenExpiresAt ? new Date(this.#authToken.refreshTokenExpiresAt) : null;
  }
  async #refreshToken() {
    if (!this.#authToken.refreshToken) {
      throw new Error("No refresh token");
    } else if (this.accessTokenExpiresAt.getTime() - 5e3 >= Date.now()) {
      return;
    } else if (this.refreshTokenExpiresAt && this.refreshTokenExpiresAt.getTime() < Date.now()) {
...
function isTwitchMeta(metadata) {
  return typeof metadata === "object" && !!metadata && "service" in metadata && metadata?.service === "twitch";
}
function twitchSettingDefinition() {
  const tokens2 = Discern.definition.discern.settings["tokens"];
  if (tokens2?.type !== "array") return null;
  const token = tokens2.items["token"];
...
// src/plugin.ts
var configureInput = await Discern.plugin.input("configure");
var abortController = new AbortController();
function describe(object, depth = 0, history = /* @__PURE__ */ new Set()) {
...
async function newSubscription(request, account) {
  if (account.requests.some(
    (sub) => sub.type === request.type && sub.condition === request.condition && sub.version === request.version
  )) {
    return;
  }
...
async function pollMessages(conn, account) {
...
async function initializeConnection(account, url) {
